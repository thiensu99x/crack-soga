...
#        return 0
#    fi
    bash <(curl -Ls https://raw.githubusercontent.com/thiensu99/crack-soga/main/install.sh) $version
    if [[ $? == 0 ]]; then
...
update_shell() {
    wget -O /usr/bin/soga -N --no-check-certificate https://raw.githubusercontent.com/thiensu99/crack-soga/main/soga.sh
    if [[ $? != 0 ]]; then
        echo ""
